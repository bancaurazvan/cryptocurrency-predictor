...
def buildNet(w_init="glorot_uniform", act="tanh"):
    ...
    net = Sequential()
    net.add(Dense(12, kernel_initializer=w_init,
                  input_dim=12, activation='linear'))
    net.add(Reshape((1, 12)))
    net.add(BatchNormalization())
    net.add(GRU(40, kernel_initializer=w_init,
                activation=act, return_sequences=True))
    net.add(Dropout(0.4))
    net.add(GRU(70, kernel_initializer=w_init,
                activation=act, return_sequences=True))
    net.add(Dropout(0.3))
    net.add(GRU(70, kernel_initializer=w_init,
                activation=act, return_sequences=True))
    net.add(Dropout(0.4))
    net.add(GRU(40, kernel_initializer=w_init,
                activation=act, return_sequences=False))
    net.add(Dropout(0.4))
...
def predictFuture(m1, m2, old_pred, writeToFile=False):
    ...
    if writeToFile:
        f = open("results", "a")
        f.write("[{}] Actual:{}$ Last Prediction:{}$ Next 9m:{}$".format(
            time.strftime("%H:%M:%S"), latest_p, old_pred, pred))
        f.close()

    c = conn.cursor()
    c.execute("INSERT INTO predict(actual,last,target,cex_ask,slope,nrmse) VALUES (?,?,?,?,?,?)",
              (latest_p, old_pred, pred, cex["ask"], slope, nrmse))
    conn.commit()
    print("[{}] Actual:{}$ Last Prediction:{}$ Next 9m:{}$ Slope:{}$ NRMSE:{}$\n".format(
        time.strftime("%H:%M:%S"), latest_p, old_pred, pred, slope, nrmse))
...
if __name__ == '__main__':
    ...
    if args.run is not None:
        ...
        print("training...")
        el = len(data)-10
        net.fit(np.array(data[:el]), np.array(
            labels[:el]), epochs=epochs, batch_size=300)
        print("trained!\nSaving...", end="")
